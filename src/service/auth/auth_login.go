...
func AuthBasicLogin(ctx context.Context, c *AuthLoginCommand) (accessToken string, refreshToken string, role string, err error) {
	...
	err = collection.Account().Collection().FindOne(ctx, bson.M{"user_name": c.UserName}).Decode(account)
	if err != nil {
		codeErr := src_const.ServiceErr_Auth + src_const.ElementErr_Account + src_const.AccountNotFound
		service.AddError(ctx, "", "", codeErr)
		return "", "", "", fmt.Errorf("account is not existed")
	}
	role = account.Role

	if account.Status != model_account.StatusActive {
		codeErr := src_const.ServiceErr_Auth + src_const.ElementErr_Account + src_const.AccountNotActive
		service.AddError(ctx, "", "", codeErr)
	...
	if err != nil {
		codeErr := src_const.ServiceErr_Auth + src_const.ElementErr_Account + src_const.Unauthorized
		service.AddError(ctx, "", "", codeErr)
	...
	if err != nil {
		codeErr := src_const.ServiceErr_Auth + src_const.ElementErr_Account + src_const.UnableCreateToken
		service.AddError(ctx, "", "", codeErr)
